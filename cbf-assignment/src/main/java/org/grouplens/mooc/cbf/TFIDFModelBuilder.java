...
public class TFIDFModelBuilder implements Provider<TFIDFModel> {
	...
	public TFIDFModel get() {
		...
		for (VectorEntry e: docFreq.fast()) {
			...
			double idf = itemVectors.size() / e.getValue();
			docFreq.set(e.getKey(), Math.log(idf));
		}
		...
		for (Map.Entry<Long,MutableSparseVector> entry: itemVectors.entrySet()) {
			...
			for (VectorEntry vectorEntry : tv) {
				long termId = vectorEntry.getKey();
				double tf = vectorEntry.getValue();
				double idf = docFreq.get(termId);
				tv.set(termId, tf * idf);
			}            
