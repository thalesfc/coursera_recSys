...
public class TFIDFItemScorer extends AbstractItemScorer {
	...
	private SparseVector makeUserVector_unweighted(long user) {
	...
	private SparseVector makeUserVector(long user) {
		...
		for (Rating r: userRatings) {
			...
			// preferences to express the user unrating an item
			if (p != null) {
				// weigth the item according to the user scores
				// Get the item's vector and add it to the user's profile
				long itemId = p.getItemId();
				SparseVector iv = model.getItemVector(itemId);
				MutableSparseVector mutableIv = iv.mutableCopy();
				double weight = p.getValue() - meanRatings;
				mutableIv.multiply(weight);
				profile.add(mutableIv);
			}
		}
		// The profile is accumulated, return it.
		// It is good practice to return a frozen vector.
