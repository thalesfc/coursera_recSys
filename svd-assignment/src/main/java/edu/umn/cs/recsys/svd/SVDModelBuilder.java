...
public class SVDModelBuilder implements Provider<SVDModel> {
	...
	public SVDModel get() {
		...
		RealMatrix U = svd.getU().getSubMatrix(0, svd.getU().getRowDimension() - 1, 0, featureCount - 1);
		RealMatrix S = svd.getS().getSubMatrix(0, featureCount - 1, 0, featureCount - 1);
		RealMatrix V = svd.getV().getSubMatrix(0, svd.getV().getRowDimension() - 1, 0, featureCount -1);
	...
	private RealMatrix createRatingMatrix(IdIndexMapping userMapping, IdIndexMapping itemMapping) {
		...
		try {
			for (UserHistory<Event> user: users) {
				...
				ratings.subtract(baselines); // v = r - baseline

				for (VectorEntry vectorEntry : ratings.fast(VectorEntry.State.EITHER)) {
					int i = itemMapping.getIndex(vectorEntry.getKey());
					double v = vectorEntry.getValue();
					matrix.setEntry(u, i, v);
				}
